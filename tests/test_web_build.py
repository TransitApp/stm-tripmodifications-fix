...
def test_cancelled_flags_become_the_span():
    feed = static_feed()
    result = build([one_detour(range(4, 7))], feed, SERVICE_DATE)
...
def test_replacement_stops_are_ordered_along_the_detour():
    ...
    result = build([one_detour(range(4, 7), replacements)], static_feed(), SERVICE_DATE)
...
def test_a_replacement_stop_off_every_detour_is_left_out():
    ...
    result = build([one_detour(range(4, 7), far)], static_feed(), SERVICE_DATE)
...
def test_the_shape_follows_the_detour():
    result = build([one_detour(range(4, 7))], static_feed(), SERVICE_DATE)
...
def test_a_detour_that_drops_nothing_names_the_stop_it_leaves_from():
    ...
    result = build([detour], static_feed(), SERVICE_DATE)
...
def test_a_short_turn_that_serves_a_cancelled_stop_is_modified_too():
    ...
    result = build([one_detour(range(5, 8))], feed, SERVICE_DATE)
...
def test_a_pattern_with_no_trip_today_is_left_out():
    feed = static_feed()
    ...
    assert result.plans == []
    assert [item.reason for item in result.skipped] == ["no trip pattern running today is affected"]


def test_a_line_the_gtfs_does_not_have_is_reported():
    detour = one_detour(range(4, 7))
    result = build([detour], static_feed(route_id="52", direction_id="0"), SERVICE_DATE)
...
def test_the_replacement_threshold_is_configurable():
    ...
    strict = build([detour], static_feed(), SERVICE_DATE, BuildConfig())
    loose = build([detour], static_feed(), SERVICE_DATE, BuildConfig(replacement_threshold_m=300))
